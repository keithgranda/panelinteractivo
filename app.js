...
function answerQ(idx) {
  ...
  if (qIdx === shuffled.length - 1 || lives === 0) {
...
function updateLives() {
  ...
  if(qlEl) qlEl.textContent = icons.join('');
}
...
function showResult() {
  ...
  const medal = lives === 0 ? '🖤' : (pct >= 90 ? '🏆' : pct >= 70 ? '🥇' : pct >= 50 ? '🥈' : '📚');
  const msg   = lives === 0 ? '¡Te quedaste sin vidas! Repasa los conceptos e inténtalo de nuevo.' 
              : (pct >= 90 ? '¡Eres un experto en reciclaje! El planeta te necesita.'
  ...
  document.getElementById('quizWrapper').innerHTML = `
    <div style="text-align:center;padding:20px 0">
      ...
      <div style="font-family:'Baloo 2','Comic Sans MS',cursive,sans-serif;font-size:2.5rem;font-weight:800;color:var(--green)">${score} puntos</div>
